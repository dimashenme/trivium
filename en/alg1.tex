...
\begin{zadacha} Consider a complex number $z:=a+b\1$. Prove that
$z\bar z = a^2 + b^2$. That means in particular the this number is
  always nonnegative and equals zero only if $z = 0$. $z\bar z$ is
  often written down as $|z|^2$ since the length of a vector $(a,b)$
  on a plane equals $\sqrt{a^2 + b^2}$ (the distance between $z$ до
  $0$, $|z|$ is called a modulus of $z$).
\end{zadacha}
